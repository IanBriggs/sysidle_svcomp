...
struct rcu_state {
 struct rcu_node node[((1 + (3) + 0 + 0 + 0) - 3)];
...
void rcu_sysidle_force_exit(void)
{
 ...
 while (oldstate > 1) {
  newoldstate = ({ if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_begin(); } typeof(*(&full_sysidle_state)) temp = (*(&full_sysidle_state)); if ((*(&full_sysidle_state)) == (oldstate)) { (*(&full_sysidle_state)) = (0); } if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_end(); } temp; })
                                    ;
...
static void rcu_sysidle(unsigned long j)
{
 ...
 case 1:
  ...
  if (((~0UL) / 2 >= (jiffies) - (j + rcu_sysidle_delay())))
   (void)({ if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_begin(); } typeof(*(&full_sysidle_state)) temp = (*(&full_sysidle_state)); if ((*(&full_sysidle_state)) == (1)) { (*(&full_sysidle_state)) = (2); } if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_end(); } temp; })
                                              ;
  break;
 ...
 case 2:
  ...
  if (((~0UL) / 2 >= (jiffies) - (j + rcu_sysidle_delay())))
   (void)({ if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_begin(); } typeof(*(&full_sysidle_state)) temp = (*(&full_sysidle_state)); if ((*(&full_sysidle_state)) == (2)) { (*(&full_sysidle_state)) = (3); } if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_end(); } temp; })
                                             ;
  break;
...
static void rcu_sysidle_cb(struct rcu_head *rhp)
{
 ...
 rshp = ({ const typeof( ((struct rcu_sysidle_head *)0)->rh ) *__mptr = (rhp); (struct rcu_sysidle_head *)( (char *)__mptr - __builtin_offsetof (struct rcu_sysidle_head, rh) );});
...
bool rcu_sys_is_idle(void)
{
 ...
 if (rss == 3) {
  rss = ({ if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_begin(); } typeof(*(&full_sysidle_state)) temp = (*(&full_sysidle_state)); if ((*(&full_sysidle_state)) == (3)) { (*(&full_sysidle_state)) = (4); } if ("\n\tlock; "[0] != '\0') { __VERIFIER_atomic_end(); } temp; })
                                                  ;
  return rss == 3;
 }
 ...
 if (nr_cpu_ids > 4 &&
     ...
     !rsh.inuse && ({ if (""[0] != '\0') { __VERIFIER_atomic_begin(); } typeof(*(&rsh.inuse)) temp = (*(&rsh.inuse)); (*(&rsh.inuse)) = (1); if (""[0] != '\0') { __VERIFIER_atomic_end(); } temp; }) == 0)
